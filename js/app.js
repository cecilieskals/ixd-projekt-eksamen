...
// variabler der gælder hele siden
let allGames = []; 
...
function displayGame(game) {
  ...
  newCard.addEventListener("click", function () {
    showGameModal(game); // Vis modal med detaljer
  });
...
function populateGenreDropdown() {
  const genreSelect = document.querySelector("#genre-select");
  genreSelect.innerHTML = `<option value="all">Alle genre</option>`;

  const genresSet = new Set();

  for (const game of allGames) {
    // Hvis genre er et array
...
// modal til spil- et overlay med spil detaljer
